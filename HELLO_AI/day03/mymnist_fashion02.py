...
google_test_label = test_labels
google_test_image = test_images
...
for idx, label in enumerate(google_test_label):
    ai_label = np.argmax(predict_result[idx])
    if(label == ai_label):
        filename = f"test/o/{label}_{ai_label}_{idx}.png"
        cv2.imwrite(filename, google_test_image[idx])
    else:
        filename = f"test/x/{label}_{ai_label}_{idx}.png"
        cv2.imwrite(filename, google_test_image[idx])
