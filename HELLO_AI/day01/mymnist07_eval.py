...
for idx, label in enumerate(google_train_label):
    ...
    if(label == ai_label):
        filename = f"test/o/{str(label)}_{str(ai_label)}_{str(idx)}.png"
        cv2.imwrite(filename, google_train_image[idx])
    else:
        filename = f"test/x/{str(label)}_{str(ai_label)}_{str(idx)}.png"
        cv2.imwrite(filename, google_train_image[idx])
